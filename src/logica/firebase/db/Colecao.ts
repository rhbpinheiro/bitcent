...
export default class Coleca {
    ...
    async excluir(caminho: string, id?: string) {
        if (!id) {
            return false;
        }
        const db = getFirestore(app);
    ...
    async concultarComFiltros(
        ...
        const filtrosWhere =
            filtros?.map((f) => where(f.atributo, f.op, f.valor)) ?? [];
        const ordenacao = ordenarPor ? [orderBy(ordenarPor, direcao)] : []  
